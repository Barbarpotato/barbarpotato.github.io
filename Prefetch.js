...
const endpoint_list = [
	...
	{
		url: "https://api-barbarpotato.vercel.app/projects/latest",
		json_file_name: "project_latest.json",
	},
];
	...
	for (const endpoint of endpoint_list) {
		try {
			...
			const outputPath = path.resolve(
				"./public/data",
				endpoint.json_file_name,
			);

			// Save the fetched data into the respective JSON file
			fs.writeFileSync(outputPath, JSON.stringify(data, null, 2));
			console.log(
				`Data for ${endpoint.json_file_name} successfully saved to ${outputPath}`,
			);
		} catch (err) {
